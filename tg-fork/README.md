...
> **Пин:** `62b56a07` = Telegram `12.10.1 (7038)`, ветка `master`. Якоря и сигнатуры API
> сверены по этому коммиту. Тег `release-11.4.2-5469` не подходит: там экран настроек
> собран на другом фреймворке (нет `SettingCell`/`UItem`), и правку нужно портировать.
